...
int main()
{
	...
	//Exception Cases
     ...
     ETRY;
 //Add word without existing line
     ...
     ETRY;
 //Get num words from a line that doesn't exist
 ...
 //Add 'Hello' to first line
         ...
         ETRY;
 //Add 'World' to first line, count number of words
 ...
 //Make sure first word is Hello
         ...
         ETRY;
 //Try to get a word from a line that doesn't exist
         ...
         ETRY;
 //Try to get a word that doesn't exist
	
         
         exceptionThrown= 1;
         cew_Test_Count=cew_Test_Count+1;
         TRY {
         	LSAddLine();LSAddWord("Wut");
         exceptionThrown = 0;
	...
	for(i = 0; i < 1000; i++){
	    for(k = 0; k < NUMLINES; k++){
         ...
         exceptionThrown = 0;
            ...
            if ((LSAddWord(lineList[k].wordList[j])) != (KWSUCCESS)) {
               cew_Error_Count=cew_Error_Count+1;
               printf("\nTest Case ERROR (Ncase) in script at line number %d\n", 401);
               printf("Actual Value is %d\n", LSAddWord(lineList[k].wordList[j]));
               printf("Expected Value is %d\n", KWSUCCESS);
            } 
         }
